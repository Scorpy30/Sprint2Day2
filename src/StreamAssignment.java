...
class StreamAssignment {
    public static void main(String[] args) {
        List<Employee> employees = List.of(
                new Employee("Tarush", "(NO LAST NAME)", "IT", "MALE", 101, 60000, "tar@xyz.com", new Address("Tokyo", "Japan")),
                new Employee("Ankita", "Kumari", "HR", "FEMALE", 102, 55000, null, null),
                new Employee("Sayna", "Anand", "Finance", "FEMALE", 103, 70000, "say@xyz.com", new Address("London", "UK")),
                new Employee("Soham", "Chakraborty", "IT", "MALE", 104, 75000, "soh@xyz.com", new Address("Sydney", "Australia"))
        );
        ...
        List<Employee> notInIT = employees.stream()
        ...
        while (tenEmployees.size() < 10) 
            tenEmployees.addAll(employees);
        ...
        Map<String, List<String>> genderNames = employees.stream()
            .collect(Collectors.groupingBy(e -> e.gender, Collectors.mapping(Employee::getFullName, Collectors.toList())));
        ...
        // 14. Optional fields
        employees.stream()
            .map(e -> Optional.ofNullable(e.email)
                 .orElse("no-email@domain.com"))
            .forEach(System.out::println);

        // 15. Default address
        employees.stream()
            .map(e -> Optional.ofNullable(e.address)
                 .orElse(new Address("Unknown", "Unknown")))
            .forEach(System.out::println);

        // 16. Raise salary if IT
        employees.stream()
            .filter(e -> Optional.ofNullable(e.department)
                    .orElse("").equals("IT"))
            .forEach(e -> e.salary += 5000);

        // 17. Sort by city then country
        List<Address> addresses = employees.stream()
            .map(e -> Optional.ofNullable(e.address)
                 .orElse(new Address("", "")))
            .collect(Collectors.toList());
        addresses.stream()
        ...
        System.out.println("Any HR? " + employees.stream()
                           .anyMatch(e -> e.department.equals("HR")));
        System.out.println("All have email? " + employees.stream()
                           .allMatch(e -> e.email != null));
        System.out.println("None has null name? " + employees.stream()
                           .noneMatch(e -> e.firstName == null));
    }
}
